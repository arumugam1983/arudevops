  ...
  try {
    ...
    if(env.BRANCH_NAME == 'master'){
      ...
      if(env.BRANCH_NAME == 'develop'){
        ...
        stage('Deploy') {
           sh 'curl -u admin:admin -T target/**.war "http://localhost:9091/manager/text/deploy?path=/devops&update=true"'
        }

        stage("Smoke Test"){
           sh "curl --retry-delay 10 --retry 5 http://localhost:9091/devops"
        }
      ...
      if(env.BRANCH_NAME ==~ /release.*/){
        ...
        stage('Release Build And Upload Artifacts') {
         ...
         stage('Deploy To Dev') {
            sh 'curl -u jenkins:jenkins -T target/**.war "http://localhost:9091/manager/text/deploy?path=/devops&update=true"'
         }
         ...
         stage("Deploy from Artifactory to QA"){
           ...
           sh 'curl -u admin:admin -T *.war "http://localhost:9091/manager/text/deploy?path=/devops&update=true"'
         }
